...
//  cudaMallocManaged(&q,sizeof(*QNODE));
 // q= s->tail = s->head ;
...
*
*/ 
#ifndef _init_xlatcase
#define _init_xlatcase
  static void
init_xlatcase () 
{
  int i;
...
CopyMatchListEntry (ACSM_PATTERN * px) 
{
 ...
 cudaMallocManaged(&p,sizeof(ACSM_PATTERN));
            ...
            int offset, int depth, void * id, int iid) 
{
  ...
  //plist = (ACSM_PATTERN *) AC_MALLOC (sizeof (ACSM_PATTERN));
	cudaMallocManaged(&plist,sizeof(ACSM_PATTERN));
  MEMASSERT (plist, "acsmAddPattern");
  //plist->patrn = (unsigned char *) AC_MALLOC (n);  
	cudaMallocManaged(&(plist->patrn),sizeof(unsigned char)*n);
  //ConvertCaseEx (plist->patrn, pat, n);	
  memcpy(plist->patrn,pat,n);
  //plist->casepatrn = (unsigned char *) AC_MALLOC (n); 
	cudaMallocManaged(&(plist->casepatrn),sizeof(unsigned char)*n);
...
//                                        acsm->acsmMaxStates);
 cudaMallocManaged(&(acsm->acsmStateTable),sizeof(ACSM_STATETABLE) * acsm->acsmMaxStates);
            ...
            int (*Match) (void *id, int index, void *data), void *data) 
{
	...
	//printf("%d,%d,%d,%d\n",state,*T,*Tend,counttime);
     //         Match (NULL,state, data);
   ...
   //           index = T - mlist->n + 1 - Tc;
                ...
                {*/
                        nfound++;
			//Match (mlist->id, mlist->iid, data);
                        //printf("%d \t",mlist->offset);
			
			
		    if (Match (mlist->id, mlist->iid, data))	
		    {
			    return nfound;
			    //return *T;
	    	    }
...
main (int argc, char **argv) 
{
  ...
  matches=acsmSearch (acsm, (unsigned char*)text, strlen (text), MatchFound, (void *) 0);
